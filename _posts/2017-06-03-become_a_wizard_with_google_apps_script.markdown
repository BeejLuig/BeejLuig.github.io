...
In this post we are going to build a to-do list flow combining Gmail, Google Sheets and Google Calendar. The idea is this: you send yourself an email with "to do" or "todo" in the subject. The body of the email will have a new task. A spreadsheet will be periodically checking for emails in your inbox with that subject line, and when it finds one (or more), it will add a new row to the sheet with the given task. At the beginng of every day, a new calendar event will be added to your default calendar with the whole list of to-dos in the description. That's a whole bunch of integration, but in less than 25 lines of code! 
...
> **Quick Tip**: the [Apps Script docs](https://developers.google.com/apps-script/reference/calendar/) are great, keep them handy!
